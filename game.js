...
let nightTexture, nightOffset = 0;
function init() {
  ...
  nightTexture = texLoader.load("night.jpg");
  nightTexture.wrapS = THREE.RepeatWrapping;
  nightTexture.wrapT = THREE.RepeatWrapping;
  nightTexture.repeat.set(2, 1);
...
function createPanel(x, z, rotY) {
  ...
  const tex =
    billboardTextures[Math.floor(Math.random() * billboardTextures.length)];
  ...
  if (player && gameStarted && !gameOver) {
    ...
    laneLines.forEach((line) => {
      ...
      if (line.position.z > player.position.z + 5)
        line.position.z -=
          (lineDashLength + lineGap) * (laneLines.length / 2);
    });

    galleryPanelsLeft.forEach((p) => {
      p.position.z += speed;
      if (p.position.z > 10)
        p.position.z -= GALLERY_COUNT_PER_SIDE * GALLERY_SPACING;
    });
    ...
    for (let i = milkCartons.length - 1; i >= 0; i--) {
      ...
      if (
        Math.abs(player.position.x - m.position.x) < 1 &&
        Math.abs(player.position.z - m.position.z) < 1
      ) {
